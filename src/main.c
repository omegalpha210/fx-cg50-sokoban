...
{(void)context;gint_osmenu();}
static void power_off(void *context)
{(void)context;gint_poweroff(true);}
...
int main(void)
{
    ...
    barrier();sok_render(&app);
    for(;;) {
        /* Raw transformed keydev events retain releases, block while idle,
        ...
        key_event_t event=keydev_read(keydev_std(),true,NULL);
        SokKey key=SK_NONE;
